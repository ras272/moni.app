-- =====================================================
-- Migration: Fix Dashboard Calculations
-- Description: Corregir cálculos incorrectos en get_dashboard_data()
-- Created: 2025-11-16
-- Version: 1.0
-- =====================================================
--
-- FIXES:
-- 1. Porcentaje de categorías: usar gastos totales en vez de "budget" confuso
-- 2. Pagos pendientes: filtrar solo grupos del usuario actual
-- 3. Eliminar balance diario confuso (solo retornar ingresos/gastos)
--
-- =====================================================
...
-- =====================================================
-- Recrear función get_dashboard_data() con fixes
-- =====================================================
...
BEGIN
  ...
  -- ✅ FIX: Pagos pendientes - solo grupos donde el usuario es owner o participante
  WITH user_groups AS (
    SELECT mtg.id
    FROM money_tag_groups mtg
    WHERE mtg.owner_profile_id = v_profile_id
      AND mtg.is_settled = false
    UNION
    SELECT gp.group_id
    FROM group_participants gp
    WHERE gp.profile_id = v_profile_id
      AND EXISTS (
        SELECT 1 FROM money_tag_groups mtg
        WHERE mtg.id = gp.group_id AND mtg.is_settled = false
      )
  )
  ...
  -- ✅ FIX: Porcentaje basado en gastos totales, no en "budget" confuso
  -- =====================================================
  ...
  SELECT COALESCE(
    json_agg(
      json_build_object(
        ...
        'percentage', CASE
          WHEN v_current_month_expenses > 0
          THEN ROUND((ct.total_amount / v_current_month_expenses) * 100, 1)
          ELSE 0
        END,
...
FIXES aplicados:
- Porcentaje de categorías: ahora usa gastos_totales en vez de budget confuso
- Pagos pendientes: filtrado solo para grupos del usuario actual
- Top categorías: aumentado a top 5
Reduce 12+ queries individuales a 1 sola, mejorando performance de ~4s a ~500ms.';

COMMIT;

-- =====================================================
-- TESTING (ejecutar después de la migración)
-- =====================================================

-- 1. Verificar que la función se actualizó:
-- SELECT get_dashboard_data();

-- 2. Verificar porcentajes de categorías (deberían sumar ~100%):
-- SELECT
--   (data->'top_categories')::jsonb
-- FROM (SELECT get_dashboard_data() as data) t;

-- 3. Verificar pagos pendientes (solo tuyos):
-- SELECT
--   (data->'sidebar_stats'->>'pending_payments')::int
-- FROM (SELECT get_dashboard_data() as data) t;
